...
export class UIManager {
     ...
     * フィードアイテム要素を作成する
     */
    createFeedItemElement(feed, item) {
     ...
     * フィードアイテムを選択する
     */
    async selectFeedItem(feedId, itemGuid, itemTitle) {
        // 既存の選択を解除
        ...
        // 新しいアイテムを選択
        const itemElement = this.feedList.querySelector(`[data-feed-id="${feedId}"][data-item-guid="${itemGuid}"]`);
        if (itemElement) {
    ...
    async handleSaveMemo() {
        ...
        try {
            await saveMemo(this.appState.selectedItem.feedId, this.appState.selectedItem.itemGuid, content);
            alert('メモを保存しました。');
            this.updateMemoButtons();
        }
        catch (error) {
    ...
    async handleClearMemo() {
        ...
        try {
            await deleteMemo(this.appState.selectedItem.feedId, this.appState.selectedItem.itemGuid);
            this.memoTextarea.value = '';
            this.updateMemoButtons();
            alert('メモを削除しました。');
        }
