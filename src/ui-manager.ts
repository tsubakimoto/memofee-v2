import type { Feed, FeedItem, Memo, AppState } from './types.js';
...
export class UIManager {
   ...
   * 記事要素を作成する
   */
  private createArticleElement(feedId: string, item: FeedItem): HTMLElement {
   ...
   * 記事を選択する
   */
  private selectArticle(feedId: string, item: FeedItem): void {
    // 以前の選択を解除
    ...
    // 新しい選択を設定
    const articleElement = this.feedList.querySelector(
      `[data-feed-id="${feedId}"][data-item-guid="${item.guid}"]`
    );
    if (articleElement) {
   ...
   * メモエリアを更新する
   */
  private updateMemoArea(feedId: string, item: FeedItem): void {
